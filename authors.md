# Authors and Citation

## Authors

- **Marcus Beck**. Author, maintainer.
  [](https://orcid.org/0000-0002-4996-0059)

- **Perry de Valpine**. Author.

- **Rebecca Murphy**. Author.

- **Ian Wren**. Author.

- **Ariella Chelsky**. Author.

- **Melissa Foley**. Author. [](https://orcid.org/0000-0002-5832-6404)

- **David Senn**. Author. [](https://orcid.org/0000-0002-4869-3550)

## Citation

Source:
[`inst/CITATION`](https://github.com/tbep-tech/wqtrends/blob/v1.5.2/inst/CITATION)

Beck, M.W., de Valpine, P., Murphy, R., Wren, I., Chelsky, A., Foley,
M., Senn, D.B. (2022). “Multi-scale trend analysis of water quality
using error propagation of Generalized Additive Models.” *Science of the
Total Environment*, **802**, 149927.
[doi:10.1016/j.scitotenv.2021.149927](https://doi.org/10.1016/j.scitotenv.2021.149927).

    @Article{,
      title = {Multi-scale trend analysis of water quality using error propagation of Generalized Additive Models},
      author = {{Beck} and {M.W.} and {de Valpine} and {P.} and {Murphy} and {R.} and {Wren} and {I.} and {Chelsky} and {A.} and {Foley} and {M.} and {Senn} and {D.B.}},
      journal = {Science of the Total Environment},
      year = {2022},
      volume = {802},
      pages = {149927},
      doi = {10.1016/j.scitotenv.2021.149927},
    }

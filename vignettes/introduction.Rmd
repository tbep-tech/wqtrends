...
The `anlz_gam()` function is used to fit the model.  First, the raw data are filtered to select only station 34 and the chlorophyll parameter.  The model is fit using a log-10 transformation of the response variable.  Available transformation options are log10 (`log10`) or identity (`ident`). 
...
Statistical tests for evaluating trends are available in this package. These methods are considered "secondary" analyses that use results from a fitted GAM to evaluate trends or changes over time.  In particular, significance of changes over time are evaluated using mixed-effect meta-analysis applied to the GAM results to allow for full propagation of uncertainty between methods.  Each test includes a plotting method to view the results.
...
The `anlz_avgseason()`, `anlz_metseason()`, `anlz_mixmeta()`, and `show_metseason()` functions evaluate seasonal averages between years, including an assessment of the trend for selected years using mixed-effects meta-analysis modelling.  These functions require inputs for the seasonal ranges to evaluate (`doyend`, `doystr`) and years for assessing the trend in the seasonal averages/metrics (`yrstr`, `yrend`).  
...
The `anlz_metseason()` function is a more generic function that can estimate seasonal metrics and their uncertainties for any summary function available in R.  This may include seasonal maxima (`max`), minima (`min`), variance (`var`), or others.  The function uses repeated resampling of the GAM model coefficients to simulate multiple time series as an estimate of uncertainty for the summary parameter.  It is noted that this function will produce results that converge to those for `anlz_avgseason()` if the `mean` summary function is used and the number of simulations is very high.  

The inputs for `anlz_metseason()` are similar as `anlz_avgseason()`, with the addition of the `metfun` and `nsim` arguments to specify the summary function and number of simulations, respectively.  Here we show the estimate for the maximum chlorophyll in each season, using a relatively low number of simulations.  Repeating this function will produce similar but slightly different results because the estimates are stochastic. 
...
The `anlz_mixmeta()` function uses results from `anlz_avgseason()` or `anlz_metseason()` to estimate the trend in the seasonal metric over a selected year range. Here, we evaluate the seasonal trend from 2006 to 2017 for the seasonal estimate of the model results above.  

```{r}
anlz_mixmeta(avgseason, yrstr = 2006, yrend = 2017)
```

The `show_metseason()` function plots the seasonal metrics and trends over time. The `anlz_avgseason()`, `anlz_metseason()`, and `anlz_mixmeta()` functions are used internally to get the predictions.

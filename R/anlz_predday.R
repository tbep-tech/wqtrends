...
#'   filter(param %in% 'chl')
#' \dontrun{
#' anlz_predday(tomod, trans = 'boxcox')
#' }
...
#'   )
#' anlz_predday(mods = mods)
anlz_predday <- function(moddat = NULL, mods = NULL, ...) {
  ...
  out <- mods %>%
    ...
    mutate(prddat = purrr::map(modv, function(modv){
